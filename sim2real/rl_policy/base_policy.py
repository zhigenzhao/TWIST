...
class BasePolicy:
    ...
    #     current_obs_dict = {}
    #     for key in self.obs_dict:
    #         obs_list = list(self.obs_dict[key])
    ...
    def parse_current_obs_dict(self, current_obs_buffer_dict):
        """Parse observation buffer into observation dictionary."""
        import numpy as _np  # safe local alias

        current_obs_dict = {}  # <-- make sure this line exists
        for key in self.obs_dict:
            obs_list = sorted(self.obs_dict[key])
            pieces = []
            for obs_name in obs_list:
                arr = current_obs_buffer_dict[obs_name] * self.obs_scales[obs_name]
                arr = _np.asarray(arr)
                # Normalize every feature to row shape (1, N)
                if arr.ndim == 1:
                    arr = arr.reshape(1, -1)
                pieces.append(arr.astype(_np.float32))
            current_obs_dict[key] = _np.concatenate(pieces, axis=1)

        # One-time debug: verify widths and total
    ...
    def prepare_obs_for_rl(self, robot_state_data):
        ...
        return {"obs": self.obs_buf_dict["actor_obs"].astype(np.float32)}
