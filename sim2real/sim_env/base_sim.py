...
class BaseSimulator:
    ...
    def init_config(self):
        ...
        self.rate.warn = False

        # # --- Control-rate decimation (match training 50 Hz) -----------------
        # self.control_dt = self.config.get("CONTROL_DT", self.sim_dt)        # <<< ADDED
        # self.steps_per_action = max(1, int(round(self.control_dt / self.sim_dt)))  # <<< ADDED
        # self._action_hold = 0                                               # <<< ADDED
        # self._recompute_action = True                                       # <<< ADDED
        # # -------------------------------------------------------------------
    ...
    def init_scene(self):
        ...
        self.mj_model = mujoco.MjModel.from_xml_path(os.path.abspath(self.config["ROBOT_SCENE"]))
        ...
        if self.config["ENABLE_ELASTIC_BAND"]:
            ...
            self.viewer = mujoco.viewer.launch_passive(self.mj_model, self.mj_data, key_callback=self.elastic_band.MujuocoKeyCallback)
        else:
        ...
        if self.config["USE_JOYSTICK"]:
            ...
            if sys.platform == "linux":
                if self.config["SDK_TYPE"] == "unitree":
                    self.robot_bridge.SetupJoystick(device_id=self.config["JOYSTICK_DEVICE"], js_type=self.config["JOYSTICK_TYPE"])
                else:
    ...
    def sim_step(self):
        ...
        if self.config["ENABLE_ELASTIC_BAND"]:
            if self.elastic_band.enable:
                self.mj_data.xfrc_applied[self.band_attached_link, :3] = self.elastic_band.Advance(self.mj_data.qpos[:3], self.mj_data.qvel[:3])
        self.compute_torques()
        # Recompute torques only on action-update ticks; otherwise hold           # <<< ADDED
        # if self._recompute_action:                                                # <<< ADDED
        #     self.compute_torques()                                                # <<< ADDED
    ...
    def simulation_thread(self):
        ...
        while self.viewer.is_running():
            # # Decide whether to refresh the action this tick                      # <<< ADDED
            # if self._action_hold == 0:                                            # <<< ADDED
            #     self._recompute_action = True                                     # <<< ADDED
            #     self._action_hold = self.steps_per_action - 1                     # <<< ADDED
            # else:                                                                 # <<< ADDED
            #     self._recompute_action = False                                    # <<< ADDED
            #     self._action_hold -= 1
            self.sim_step()
            ...
            if sim_cnt % 1000 == 0:
                end_time = time.time()
                self.logger.info(str.format("FPS: {0:.2f}", 1000 / (end_time - start_time)))
                start_time = end_time
            self.rate.sleep()
