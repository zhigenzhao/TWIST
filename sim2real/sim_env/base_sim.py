...
class BaseSimulator:
    ...
    def init_scene(self):
        ...
        self.mj_model = mujoco.MjModel.from_xml_path(
            os.path.abspath(self.config["ROBOT_SCENE"])
        )
        ...
        if self.config["ENABLE_ELASTIC_BAND"]:
            ...
            self.band_attached_link = self.mj_model.body(
                band_attached_link_name).id
    ...
    def init_robot_bridge(self):
        ...
        if self.config["USE_JOYSTICK"]:
            ...
            if sys.platform == "linux":
                ...
                self.logger.warning(
                    "Joystick is not supported on Windows or MacOS.")
            ...
            try:
                for i in range(self.robot_bridge.num_motor):
                    self.torques[i] = (
                        motor_cmd[i].tau
                        + motor_cmd[i].kp
                        * (motor_cmd[i].q - self.mj_data.qpos[7 + i])
                        + motor_cmd[i].kd
                        * (motor_cmd[i].dq - self.mj_data.qvel[6 + i])
                    )
            except Exception as e:
                self.logger.error(str.format(
                    "Joint {0} not found in motor_cmd: {1}", i, e))
        # Set the torque limit
        self.torques = np.clip(
            self.torques, -self.robot_bridge.torque_limit, self.robot_bridge.torque_limit
        )
        ...
        while self.viewer.is_running():
            ...
            if sim_cnt % 100 == 0:
                ...
                self.logger.info(str.format(
                    "FPS: {0:.2f}", 100 / (end_time - start_time)))
                start_time = end_time
            self.rate.sleep()
